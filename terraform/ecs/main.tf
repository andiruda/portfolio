# AWS ECS deployment for portfolio site
...
resource "aws_security_group" "alb" {
  ...
  description = "Allow HTTP/HTTPS for portfolio ALB"
...
# Route 53 hosted zone (look up by domain if zone_id not provided)
...
locals {
  route53_zone_id = var.route53_zone_id != null ? var.route53_zone_id : try(data.aws_route53_zone.lookup[0].zone_id, null)
}
...
# Route 53 A record for root domain (alias to ALB)
resource "aws_route53_record" "portfolio" {
  count   = local.route53_zone_id != null && var.create_root_record ? 1 : 0
...
# www subdomain redirect (optional - points www to root)
